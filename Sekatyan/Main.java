...
//        // new TaxiでTaxiクラスのコンストラクタを呼び出して、Taxiのインスタンスをメモリ上に作り、そのアドレスの値をv[0]に代入している
//        // v[1]には生成されたTrainのインスタンスの、メモリ上のアドレスが代入されている
//        // サブクラスをスーパークラスVehicleに代入しているため、自動で変換が行われる（キャストをする必要がない）
//        v[0] = new Taxi(4);
//        v[1] = new Train(160);
//        v[2] = new Bus(80);
//
//        for (int i = 0; i < v.length; i++) {
//            // VehicleクラスのshowCrewNumメソッドを呼び出す（サブクラスでオーバーライドされているため、サブクラスのほうが呼び出される）
//            v[i].showCrewNum();
//            // v[i]がStopableインターフェースのインスタンスかどうか（その処理が実装implementsされているか）を判定している
//            if (v[i] instanceof Stopable) {
//                // Vehicleクラスにはstopメソッドがなく呼び出せない(定数crewNumと抽象メソッドのshowCrewNum()しか呼び出せない）
//                // そのため、Stopableインターフェースにキャストして呼び出している
